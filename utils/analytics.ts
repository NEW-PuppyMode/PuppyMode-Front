...
/**
 * 버튼 탭 = "시도". 설계 이벤트(logEvent)는 "성공"만 남긴다.
 *
 * 처음에는 모든 버튼에 붙여 시도 대비 성공을 보려 했는데, 막상 로그를 보니
 * 대부분의 화면에서 두 이벤트가 같은 순간에 같은 사실을 말할 뿐이었다. 그래서
 * 설계 이벤트가 덮는 곳에서는 걷어냈다.
 *
 * 남겨둘 만한 곳은 시도와 성공이 실제로 자주 갈리는 지점이다. 지금은 소셜
 * 로그인뿐이다(중도 취소·SDK 실패가 잦아 그 차이가 곧 로그인 이탈이다).
 * 아직 설계 이벤트가 없는 화면을 둘러볼 때 임시로 쓰기에도 좋다.
 */
